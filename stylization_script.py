...
def stylize_static_image(inference_config):
    ...
    training_state = torch.load(os.path.join(inference_config["model_binaries_path"], inference_config["model_name"]), map_location=device)
...
inference_config['content_input'] = 'content.jpg'
inference_config['batch_size'] = 5
inference_config['img_width'] = 600
inference_config['model_name'] = 'mosaic_4e5_e2.pth'
inference_config['should_not_display'] = None
inference_config['verbose'] = None
inference_config['redirected_output'] = None
...
def run(contentname,stylename):
    inference_config['content_input'] = contentname
    inference_config['model_name'] = stylename
